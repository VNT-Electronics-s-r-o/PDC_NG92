...
{
    lv_obj_t * target = lv_obj_create(lv_screen_active());    
    lv_obj_set_size(target, 320, 220);
...
void SubMainScreen_BarGraphTitle(lv_obj_t * target, lv_obj_t * dependOn)
{
    //Grey title
    target = lv_obj_create(lv_screen_active());
    lv_obj_set_style_bg_color(target, lv_color_hex(0xCCCCCC), LV_PART_MAIN);        
...
void Main_Screen(void)
{
    ...
    //lv_obj_t * contRowBar = SubMainScreen_BarGraph(contRowStatus);
